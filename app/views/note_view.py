...
def get_notes():
    ...
    try:
        ...
        if skip < 0 or limit < 1:
            return jsonify({"error": "Bad Request", "message": "skip >= 0, limit >= 1"}), 400

        stmt = select(Note, User.login).join(User, Note.id_user == User.user_id).offset(skip).limit(limit)
        ...
        db.session.rollback()
        return jsonify({"error": "Internal Server Error", "message": "Ошибка БД"}), 500
